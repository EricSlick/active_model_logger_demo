...
ActiveRecord::Base.connection.execute("INSERT INTO active_model_logs (loggable_type, loggable_id, message, metadata, created_at, updated_at) VALUES ('User', #{user.id}, 'System configuration', '{\"status\":\"completed\",\"category\":\"config\",\"config\":{\"database\":{\"host\":\"localhost\",\"port\":5432,\"ssl\":true},\"cache\":{\"redis\":{\"enabled\":true,\"ttl\":3600},\"memcached\":{\"enabled\":false}},\"features\":{\"beta\":{\"enabled\":true,\"users\":[\"admin\"]}}}}', datetime('now'), datetime('now'))")
...
puts 'Logs with both email and push: ' + ActiveModelLogger::Log.with_keys('email', 'push').count.to_s
